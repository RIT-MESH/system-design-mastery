...
Standalone sources under `diagrams/case-studies/intelligent-syslog-monitoring/`: `context.mmd`, `request-sequence.mmd`, `failure-flow.mmd`, `scaling-evolution.mmd`. Additional diagrams:

```mermaid
%% origin: original to system-design-mastery
sequenceDiagram
  participant P0 as Network devices
  participant P1 as Redundant collectors UDP TCP
  P0 ->> P1: request
  P1 -->> P0: response
```
...
flowchart LR
  ...
  S2["Stage 2 AI classification runbook RAG correlation."]
  S3["Stage 3 multi-site, offline operation, DR."]
  S4["Stage 4 multi-model routing, voice-agent, air-gapped"]
  S1 --> S2
  S2 --> S3
  S3 --> S4
```
