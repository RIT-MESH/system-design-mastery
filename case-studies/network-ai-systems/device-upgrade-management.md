...
Standalone sources under `diagrams/case-studies/device-upgrade-management/`: `context.mmd`, `request-sequence.mmd`, `failure-flow.mmd`, `scaling-evolution.mmd`. Additional diagrams:

```mermaid
%% origin: original to system-design-mastery
sequenceDiagram
  participant P0 as Inventory firmware tracking
  participant P1 as Upgrade planner risk analysi
  P0 ->> P1: request
  P1 -->> P0: response
```
...
flowchart LR
  ...
  S2["Stage 2 automated planning approval backup rollback."]
  S3["Stage 3 HA cluster-aware, dependency checks, AI risk"]
  S4["Stage 4 fleet-wide scheduled rollout, air-gapped firm"]
  S1 --> S2
  S2 --> S3
  S3 --> S4
```
